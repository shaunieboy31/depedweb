...
export default function NoticeOfMeeting() {
  const [searchQuery, setSearchQuery] = useState("");

  const mockDocs = [
    { id: 1, num: "NOM No. 005, s. 2024", title: "EMERGENCY MEETING OF ALL ELEMENTARY AND SECONDARY SCHOOL HEADS", date: "April 05, 2024", size: "280 KB" },
    { id: 2, num: "NOM No. 004, s. 2024", title: "MONTHLY COORDINATION MEETING OF THE DIVISION EXECUTIVE COMMITTEE (DEXCOM)", date: "April 01, 2024", size: "150 KB" },
  ];
      ...
      <section className="relative h-[450px] md:h-[550px] w-full flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 w-full space-y-12 mt-12">
           ...
           <div className="relative group max-w-3xl">
              ...
              <input 
                 type="text" 
                 placeholder="Search Notices of Meeting..."
                 value={searchQuery}
      ...
      <div className="max-w-5xl mx-auto px-6 -mt-32 relative z-20 space-y-8">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-4 border border-white shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
           <div className="flex items-center gap-4 text-slate-500">
              ...
              <div className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-lg text-[10px] font-black uppercase tracking-widest">Official Calls</div>
           </div>
           ...
           <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                 <thead className="bg-[#032977] text-white">
                    <tr>
                       <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] first:pl-10">Meeting Notice #</th>
                 ...
                 <tbody className="divide-y divide-slate-100">
                    {mockDocs.map((doc) => (
                      <tr key={doc.id} className="group hover:bg-indigo-50/50 transition-all duration-300">
                         <td className="px-8 py-8 first:pl-10">
                            <span className="text-sm font-black text-slate-900 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">
                               {doc.num.split(',')[0].replace('NOM No. ', '')}
                            </span>
                         </td>
                         ...
                         <td className="px-8 py-8">
                            <h3 className="text-base font-black text-slate-900 group-hover:text-indigo-700 transition-colors leading-snug uppercase tracking-tight">
                               {doc.title}
                            </h3>
                         </td>
                         <td className="px-8 py-8">
                            <button className="flex items-center gap-3 px-6 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/10 active:scale-95">
                               <FileDown size={14} />
        ...
        <div className="bg-slate-50/50 rounded-[3rem] p-16 border-2 border-dashed border-slate-200 text-center space-y-6">
           ...
           <div className="space-y-2">
              <h4 className="text-xl font-black text-slate-900 uppercase">Meeting Archive</h4>
              <p className="text-slate-500 text-sm font-medium">Official invitation and call-to-order logs for SDO Imus City.</p>
           </div>
        </div>
