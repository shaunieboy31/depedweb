...
export default function NoticeOfDistribution() {
  const [searchQuery, setSearchQuery] = useState("");

  const mockDocs = [
    { id: 1, num: "NOD No. 002, s. 2024", title: "DISTRIBUTION OF LEARNING SPACE KITS FOR ELEMENTARY SCHOOLS", date: "April 08, 2024", size: "350 KB" },
    { id: 2, num: "NOD No. 001, s. 2024", title: "ALLOCATION AND DISPATCH OF MATATAG CURRICULUM TEACHER'S GUIDES", date: "March 20, 2024", size: "1.8 MB" },
  ];
      ...
      <section className="relative h-[450px] md:h-[550px] w-full flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 w-full space-y-12 mt-12">
           ...
           <div className="relative group max-w-3xl">
              ...
              <input 
                 type="text" 
                 placeholder="Search Notices of Distribution..."
                 value={searchQuery}
      ...
      <div className="max-w-5xl mx-auto px-6 -mt-32 relative z-20 space-y-8">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-4 border border-white shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
           <div className="flex items-center gap-4 text-slate-500">
              ...
              <div className="px-3 py-1 bg-cyan-100 text-cyan-700 rounded-lg text-[10px] font-black uppercase tracking-widest">Active Dispatch</div>
           </div>
           ...
           <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                 <thead className="bg-[#032977] text-white">
                    <tr>
                       <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] first:pl-10">Dispatch Number</th>
                 ...
                 <tbody className="divide-y divide-slate-100">
                    {mockDocs.map((doc) => (
                      <tr key={doc.id} className="group hover:bg-cyan-50/50 transition-all duration-300">
                         <td className="px-8 py-8 first:pl-10">
                            <span className="text-sm font-black text-slate-900 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">
                               {doc.num.split(',')[0].replace('NOD No. ', '')}
                            </span>
                         </td>
                         ...
                         <td className="px-8 py-8">
                            <h3 className="text-base font-black text-slate-900 group-hover:text-cyan-700 transition-colors leading-snug uppercase tracking-tight">
                               {doc.title}
                            </h3>
                         </td>
                         <td className="px-8 py-8">
                            <button className="flex items-center gap-3 px-6 py-3 bg-cyan-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-cyan-700 transition-all shadow-lg shadow-cyan-500/10 active:scale-95">
                               <FileDown size={14} />
        ...
        <div className="bg-slate-50/50 rounded-[3rem] p-16 border-2 border-dashed border-slate-200 text-center space-y-6">
           ...
           <div className="space-y-2">
              <h4 className="text-xl font-black text-slate-900 uppercase">Distribution Archive</h4>
              <p className="text-slate-500 text-sm font-medium">Logistical tracking and supply allocation logs for the division.</p>
           </div>
        </div>
